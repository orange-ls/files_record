...
class WorkflowCrawler:
    ...
    def login(self, username, password):
        ...
        try:
            ...
            if login_response.status_code == 302:
                return self._follow_redirect_chain(login_response.headers['Location'])
            return False
    ...
    def _follow_redirect_chain(self, initial_location):
        """处理多级重定向"""
        # 从login界面重定向到MainPage，然后再跳转到PendingWorkItem
        try:
    ...
    def _parse_work_table(self, html):
        """解析待办事项表格"""
        try:
...
if __name__ == "__main__":
    ...
    if html:
        print("\n正在抓取待办事项...")
        result = crawler._parse_work_table(html)

...
- **禁止**将查询结果写入 .json 缓存文件到工作空间
- 查询结果直接在内存中处理，解析 JSON 后构建 Markdown 表格展示
- 如需持久化，统一导出为 Excel 文件（用户明确要求时才导出）
- 工作空间中无用的 .json 文件应定期清理或删除
...
| 文件 | 内容 | 何时查阅 |
|---|---|---|
| [references/database-schema.md](references/database-schema.md) | 表结构、字段定义、表间关系 | 生成 SQL 前必读 |
| [references/sql-generation-rules.md](references/sql-generation-rules.md) | SQL 生成规则 | 生成 SQL 时按规则执行 |
| [references/business-formulas.md](references/business-formulas.md) | 业务计算公式 | 涉及计算型查询时必读 |
| [references/refresh-methods.md](references/refresh-methods.md) | 刷新逻辑和数据来源 | 判断数据时效性时参考 |

### MCP 查询的字段顺序

MCP SQL 查询时，SELECT 的字段和顺序见 [references/mcp-query-fields.md](references/mcp-query-fields.md)，严格按定义执行，不要自己选字段、不要改顺序。
...
1. 从 [references/mcp-query-fields.md](references/mcp-query-fields.md) 找到对应表的字段列表和中文表头
...
1. 先读取索引文件 [knowledge/index.md](knowledge/index.md)，根据用户问题中的关键词定位到具体主题文件
...
执行过程中可能遇到以下错误，按对应方式处理：

### 数据库连接错误

| 错误信息 | 原因 | 处理方式 |
|---|---|---|
| `could not connect to server` | 数据库服务未启动或网络不通 | 告知用户数据库连接失败，建议检查网络或联系管理员 |
| `FATAL: password authentication failed` | 密码错误或用户权限不足 | 告知用户认证失败，建议检查 `fetchers/config.ini` 中的配置 |
| `connection refused` | 数据库端口未开放 | 告知用户连接被拒绝，建议确认数据库服务状态 |

### SQL 执行错误

| 错误信息 | 原因 | 处理方式 |
|---|---|---|
| `relation "xxx" does not exist` | 表名不存在 | 检查表名是否正确，参考 `references/database-schema.md` |
| `column "xxx" does not exist` | 字段名不存在 | 检查字段名是否正确，参考 `references/mcp-query-fields.md` |
| `syntax error at or near` | SQL 语法错误 | 检查 SQL 语法，特别注意引号、逗号、括号匹配 |
| `division by zero` | 除零错误 | 检查计算公式中的分母是否可能为 0，加 NULLIF 保护 |

### fetcher 执行错误

| 错误信息 | 原因 | 处理方式 |
|---|---|---|
| `ModuleNotFoundError: No module named 'xxx'` | Python 模块未安装 | 告知用户缺少依赖，建议运行 `pip install xxx` |
| `FileNotFoundError: config.ini` | 配置文件不存在 | 告知用户缺少配置文件，检查 `fetchers/config.ini` 路径 |
| `KeyError: 'xxx'` | 返回数据缺少预期字段 | 检查 fetcher 的 TREE_FIELDS 和 FIELD_LABELS 是否匹配 |
| `json.decoder.JSONDecodeError` | JSON 解析失败 | 检查脚本输出格式，确保是有效的 JSON 字符串 |

### 错误处理原则

1. **不要暴露原始错误堆栈**给用户，用简明语言说明问题原因
2. **提供可操作的建议**，如"检查网络连接"、"确认表名是否存在"
3. **记录错误到知识库**，如果错误有业务价值（如某个字段含义不清楚），记录到 `knowledge/` 对应主题文件
4. **无法自行解决时**，告知用户"请联系管理员或技术支持"

"""
备料总表 fetcher
对应模型：prepare.materials（虚拟模型，无实体表）

备料总表数据由 SQL 实时计算，不存储在数据库中。
只支持查询，不支持独立刷新（依赖 bom_total_table 刷新后自动更新）。
"""
import logging
...
from ._sql_builders import build_prepare_sql

_logger = logging.getLogger(__name__)

TREE_FIELDS = [
    'bundling_number', 'material_code', 'material_desc', 'supplier_pn',
    'name', 'total_usage', 'theo_non_rate', 'city', 'sales',
    'reserve_quantity', 'stock_quantity', 'gap_quantity',
...
def query(conn, limit: int = 20, offset: int = 0, filters: dict = None,
    ...
    """
    查询备料总表（虚拟模型，实时 SQL 计算）。
    :param information_sources: 信息来源过滤（如 '存量表'），为空时走 PO与存量模式（不按信息来源分组）
    :param filters: 支持 material_code, bundling_number, name, city 等字段过滤
    """
    base_sql = build_prepare_sql(information_sources)
    ...
    if filters:
        allowed = {'material_code', 'bundling_number', 'name', 'city',
                   'stock_alert_status', 'product_category3'}
    ...
    paged_sql = f'SELECT * FROM ({base_sql}) T WHERE {where_sql} ORDER BY material_code, city LIMIT %s OFFSET %s'
    count_sql = f'SELECT COUNT(1) FROM ({base_sql}) T WHERE {where_sql}'
